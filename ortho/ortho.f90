...
program ortho
  ...
  use mod_gto, only: orthowfn, epsortho, init_gto
  ...
  write (uout,'(1x,a)') string('# |          ortho: check wfn orthonormalization         |')
  write (uout,'(1x,a)') string('#  =====================================================')
  write (uout,'(1x,a)') string('#')
  ...
  do while (getline(uin,line))
    ...
    ! Parallel options
      ...
      if (.not.ok) call ferror ('ortho', 'wrong threads line', faterr) 
    ...
    ! WFN 
      ...
      if (.not.ok) call ferror ('ortho', 'wrong load line', faterr) 
    ...
    ! Orders
      ...
      if (.not.isdata) call ferror ('ortho', 'load wfn before ortho order', faterr) 
      write (uout,'(1x,a)') string('# *** Cheking WFN file')
      call orthowfn ()

    else if (equal(word,'epsortho')) then
      ok = isreal(epsortho, line, lp)
      if (.not.ok) call ferror ('ortho', 'wrong epsortho order', faterr) 
    ...
    ! Exit main driver
      ...
      call ferror ('ortho', 'unknown option', faterr)
